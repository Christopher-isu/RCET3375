...
DurationCounter EQU 0x25 ; Counter for number of full periods per note
NoteIndex EQU 0x26 ; Index for current note in melody
...
Start:
    ...
    ; Configure PORTA: ((PORTA) and 07Fh), 0 and ((PORTA) and 07Fh), 1 outputs (for square wave)
    ...
    CLRF ANSELH ; Disable analog on PORTB (unused)
    ...
    BCF STATUS, 5 ; for PORTA access
    MOVLW 0x03 ; ((PORTA) and 07Fh), 0=1, ((PORTA) and 07Fh), 1=1
    MOVWF PORTA ; Set PORTA outputs high
    CLRF NoteIndex ; Start at first note

;=================================================================
; Play Imperial March Snippet Continuously
;=================================================================
PlaySequence:
    MOVF NoteIndex, W ; Load current note index
    CALL MelodyTable ; Get CurrentInner (W) and DurationCounter (W after second call)
    MOVWF CurrentInner ; Store note's delay value
    CALL MelodyTable ; Get DurationCounter for the note
    MOVWF DurationCounter ; Store duration
    CALL PlayNote ; Play the note
    INCF NoteIndex, F ; Move to next note
    MOVLW 0x10 ; Check if index reached 16 (8 notes × 2 entries per note)
    SUBWF NoteIndex, W
    BTFSC STATUS, 2 ; If NoteIndex == 16, reset to 0
    CLRF NoteIndex
    GOTO PlaySequence ; Loop forever

;=================================================================
; Play Note Subroutine: Loops full periods (low + high delays)
;=================================================================
PlayNote:
PlayFullPeriodLoop:
    MOVLW 0x00 ; Set ((PORTA) and 07Fh), 0 and ((PORTA) and 07Fh), 1 low
    MOVWF PORTA
    CALL DelayVariable ; Low state delay (half-period)
    MOVLW 0x03 ; Set ((PORTA) and 07Fh), 0 and ((PORTA) and 07Fh), 1 high
    MOVWF PORTA
    CALL DelayVariable ; High state delay (half-period)
    DECFSZ DurationCounter, F ; Decrement counter, skip if zero
    GOTO PlayFullPeriodLoop
    RETURN

;=================================================================
; Melody Lookup Table (note, duration pairs for Imperial March)
;=================================================================
MelodyTable:
    ADDWF PCL, F ; Add index to PCL for jump
    ; Note 1: G4, ~200 ms
    RETLW 0x1E ; G4, ~2.506 ms period (~1.253 ms half-period)
    RETLW 80h ; ~80 periods (~200.48 ms)
    ; Note 2: G4, ~200 ms
    RETLW 0x1E ; G4
    RETLW 80h
    ; Note 3: G4, ~200 ms
    ...
    RETLW 80h
    ; Note 4: Eb4, ~258 ms
    RETLW 0x27 ; Eb4, ~3.226 ms period (~1.613 ms half-period)
    RETLW 80h
    ; Note 5: Bb4, ~156 ms
    RETLW 0x17 ; Bb4, ~1.946 ms period (~0.973 ms half-period)
    RETLW 80h
    ; Note 6: G4, ~200 ms
    RETLW 0x1E ; G4
    RETLW 80h
...
;=================================================================
; Variable Delay Subroutine (~0.8ms to 3.8ms at 4MHz, based on CurrentInner)
;=================================================================
DelayVariable:
