...
Start:
    ...
    ; Configure PORTB: ((PORTB) and 07Fh), 0 -((PORTB) and 07Fh), 3 outputs (rows), ((PORTB) and 07Fh), 4 -((PORTB) and 07Fh), 7 inputs (columns)
    BCF STATUS, 6 ; Stay in Bank 1
    BSF STATUS, 5 ; for TRISB access
    MOVLW 0b11110000 ; ((PORTB) and 07Fh), 0 -((PORTB) and 07Fh), 3=0 (outputs), ((PORTB) and 07Fh), 4 -((PORTB) and 07Fh), 7=1 (inputs)
    MOVWF TRISB ; Set PORTB direction for 4x4 keypad matrix

    ; Configure PORTA: ((PORTA) and 07Fh), 0 and ((PORTA) and 07Fh), 1 outputs (for square wave mirroring)
    BCF STATUS, 6 ; Stay in Bank 1
    BSF STATUS, 5 ; for TRISA access
    MOVLW 0b11111100 ; ((PORTA) and 07Fh), 0=0, ((PORTA) and 07Fh), 1=0 (outputs), ((PORTA) and 07Fh), 2 -((PORTA) and 07Fh), 5=1 (inputs)
    ...
    ; Initialize PORTA with ((PORTA) and 07Fh), 0 and ((PORTA) and 07Fh), 1 high (default state for no sound)
    BCF STATUS, 6 ; Select Bank 0 (((STATUS) and 07Fh), 6=0, ((STATUS) and 07Fh), 5=0)
    BCF STATUS, 5 ; for PORTA/PORTB access
    MOVLW 0x03 ; ((PORTA) and 07Fh), 0=1, ((PORTA) and 07Fh), 1=1
    MOVWF PORTA ; Set PORTA outputs high
    MOVLW 0x18 ; Initial inner value for 1ms delay
    MOVWF CurrentInner; Set initial delay

Main:
    CALL DelayVariable ; Delay in current ((PORTA) and 07Fh), 0/((PORTA) and 07Fh), 1 state
    CLRF KEY ; Clear keypress flag (no key detected yet)
    CLRF DECIMAL ; Clear decimal value for highest key (0x00-0x0F)

    ; Scan Row 0 (((PORTB) and 07Fh), 0=1): keys 1-4, checking columns ((PORTB) and 07Fh), 4 -((PORTB) and 07Fh), 7
    MOVLW 0b11110000 ; Mask to clear row bits ((PORTB) and 07Fh), 0 -((PORTB) and 07Fh), 3
    ANDWF PORTB, F ; Set all rows low (disable other rows)
    MOVLW 0b00000001 ; Set ((PORTB) and 07Fh), 0 high to scan keys 1-4
...
key3:
    ...
    BSF KEY, 0 ; Set keypress flag
key4:
    BTFSS PORTB, 7 ; Check ((PORTB) and 07Fh), 7 (col3): high if key 4 pressed
...
row1:
    ; Scan Row 1 (((PORTB) and 07Fh), 1=1): keys 5-8, checking columns ((PORTB) and 07Fh), 4 -((PORTB) and 07Fh), 7
    MOVLW 0b11110000 ; Mask to clear row bits ((PORTB) and 07Fh), 0 -((PORTB) and 07Fh), 3
    ANDWF PORTB, F ; Set all rows low
    MOVLW 0b00000010 ; Set ((PORTB) and 07Fh), 1 high to scan keys 5-8
    IORWF PORTB, F ; Activate row 1
    NOP ; 1 탎 delay for column input settling
    BTFSS PORTB, 4 ; Check ((PORTB) and 07Fh), 4 (col0): high if key 5 pressed
    GOTO key6 ; Skip if not pressed
    MOVLW 0x05 ; Load key 5 value
    MOVWF DECIMAL ; Update highest key
    BSF KEY, 0 ; Set keypress flag
key6:
    BTFSS PORTB, 5 ; Check ((PORTB) and 07Fh), 5 (col1): high if key 6 pressed
    GOTO key7 ; Skip if not pressed
    MOVLW 0x06 ; Load key 6 value
    MOVWF DECIMAL ; Update highest key
    BSF KEY, 0 ; Set keypress flag
key7:
    BTFSS PORTB, 6 ; Check ((PORTB) and 07Fh), 6 (col2): high if key 7 pressed
    GOTO key8 ; Skip if not pressed
    MOVLW 0x07 ; Load key 7 value
    MOVWF DECIMAL ; Update highest key
    BSF KEY, 0 ; Set keypress flag
key8:
    BTFSS PORTB, 7 ; Check ((PORTB) and 07Fh), 7 (col3): high if key 8 pressed
...
row2:
    ; Scan Row 2 (((PORTB) and 07Fh), 2=1): keys 9-C, checking columns ((PORTB) and 07Fh), 4 -((PORTB) and 07Fh), 7
    MOVLW 0b11110000 ; Mask to clear row bits ((PORTB) and 07Fh), 0 -((PORTB) and 07Fh), 3
    ANDWF PORTB, F ; Set all rows low
    MOVLW 0b00000100 ; Set ((PORTB) and 07Fh), 2 high to scan keys 9-C
    IORWF PORTB, F ; Activate row 2
    NOP ; 1 탎 delay for column input settling
    BTFSS PORTB, 4 ; Check ((PORTB) and 07Fh), 4 (col0): high if key 9 pressed
    GOTO keyA ; Skip if not pressed
    MOVLW 0x09 ; Load key 9 value
    MOVWF DECIMAL ; Update highest key
    BSF KEY, 0 ; Set keypress flag
keyA:
    BTFSS PORTB, 5 ; Check ((PORTB) and 07Fh), 5 (col1): high if key A pressed
    GOTO keyB ; Skip if not pressed
    MOVLW 0x0A ; Load key A value
    MOVWF DECIMAL ; Update highest key
    BSF KEY, 0 ; Set keypress flag
keyB:
    BTFSS PORTB, 6 ; Check ((PORTB) and 07Fh), 6 (col2): high if key B pressed
    GOTO keyC ; Skip if not pressed
    MOVLW 0x0B ; Load key B value
    MOVWF DECIMAL ; Update highest key
    BSF KEY, 0 ; Set keypress flag
keyC:
    BTFSS PORTB, 7 ; Check ((PORTB) and 07Fh), 7 (col3): high if key C pressed
...
row3:
    ; Scan Row 3 (((PORTB) and 07Fh), 3=1): keys D-F, 0, checking columns ((PORTB) and 07Fh), 4 -((PORTB) and 07Fh), 7
    MOVLW 0b11110000 ; Mask to clear row bits ((PORTB) and 07Fh), 0 -((PORTB) and 07Fh), 3
    ANDWF PORTB, F ; Set all rows low
    MOVLW 0b00001000 ; Set ((PORTB) and 07Fh), 3 high to scan keys D-F, 0
    IORWF PORTB, F ; Activate row 3
    NOP ; 1 탎 delay for column input settling
    BTFSS PORTB, 4 ; Check ((PORTB) and 07Fh), 4 (col0): high if key D pressed
    GOTO keyE ; Skip if not pressed
    MOVLW 0x0D ; Load key D value
    MOVWF DECIMAL ; Update highest key
    BSF KEY, 0 ; Set keypress flag
keyE:
    BTFSS PORTB, 5 ; Check ((PORTB) and 07Fh), 5 (col1): high if key E pressed
    GOTO keyF ; Skip if not pressed
    MOVLW 0x0E ; Load key E value
    MOVWF DECIMAL ; Update highest key
    BSF KEY, 0 ; Set keypress flag
keyF:
    BTFSS PORTB, 6 ; Check ((PORTB) and 07Fh), 6 (col2): high if key F pressed
    GOTO key0 ; Skip if not pressed
    MOVLW 0x0F ; Load key F value
    MOVWF DECIMAL ; Update highest key
    BSF KEY, 0 ; Set keypress flag
key0:
    BTFSS PORTB, 7 ; Check ((PORTB) and 07Fh), 7 (col3): high if key 0 pressed
...
AfterScan:
    ...
    GOTO Main ; No press: back to delay (((PORTA) and 07Fh), 0/((PORTA) and 07Fh), 1 high) and rescan
...
GetDelay:
    ...
    MOVLW 0x00 ; Set ((PORTA) and 07Fh), 0 and ((PORTA) and 07Fh), 1 low
    MOVWF PORTA ; Write to PORTA
    CALL DelayVariable ; Hold low for variable ms
    MOVLW 0x03 ; Set ((PORTA) and 07Fh), 0 and ((PORTA) and 07Fh), 1 high
    MOVWF PORTA ; Write to PORTA
