...
# 1 "i2c_MASTER.S" 2
;=================================================================
; PIC16F883 I2C MASTER, send 1 test byte to slave 0x20
;=================================================================
...
;-----------------------------------------------------------------
; Send 1 byte to slave 0x20
; address byte = 0x40 (write)
;-----------------------------------------------------------------
Send1Byte:
    ; START
    call I2C_WaitIdleM
    BANKSEL SSPCON2
    bsf SSPCON2,0 ; ((SSPCON2) and 07Fh), 0
WaitStartM:
    btfsc SSPCON2,0
    goto WaitStartM
...
WaitAddrAckM:
    BANKSEL PIR1
    btfss PIR1,3 ; ((PIR1) and 07Fh), 3
    goto WaitAddrAckM
    bcf PIR1,3
    BANKSEL SSPCON2
    btfsc SSPCON2,6 ; ((SSPCON2) and 07Fh), 6=1 => NACK
    goto StopOnly
...
StopOnly:
    ; STOP
    BANKSEL SSPCON2
    bsf SSPCON2,2 ; ((SSPCON2) and 07Fh), 2
WaitStopM:
    btfsc SSPCON2,2
    goto WaitStopM
    return
...
Start:
    ...
    ; TRIS for I2C
    BANKSEL TRISC
    movlw 0b00011000 ; ((PORTC) and 07Fh), 3/((PORTC) and 07Fh), 4 inputs
    movwf TRISC
    ...
    ; MSSP master, 100 kHz @ 4 MHz: SSPADD = 9
    BANKSEL SSPSTAT
    movlw 0x80 ; ((SSPSTAT) and 07Fh), 7=1, slew off
    movwf SSPSTAT
    BANKSEL SSPCON
    movlw 0x28 ; I2C master mode, ((SSPCON) and 07Fh), 5=1
    movwf SSPCON
    BANKSEL SSPADD
    movlw 9
    movwf SSPADD

    ; Clear flags, disable MSSP IRQ (polling)
    BANKSEL PIR1
    bcf PIR1,3
    BANKSEL PIE1
    bcf PIE1,3
