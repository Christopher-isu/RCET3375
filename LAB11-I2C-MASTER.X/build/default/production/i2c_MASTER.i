...
I2C_IdleLoop:
    ...
    andlw 0x1F ; mask bits 0-4: ((SSPCON2) and 07Fh), 0,((SSPCON2) and 07Fh), 1,((SSPCON2) and 07Fh), 2,((SSPCON2) and 07Fh), 3,((SSPCON2) and 07Fh), 4
...
I2C_Start:
    ...
    BANKSEL SSPCON2 ; select bank containing SSPCON2
    bsf SSPCON2,0 ; set ((SSPCON2) and 07Fh), 0 bit ? initiate START condition
I2C_WaitStart:
    btfsc SSPCON2,0 ; test ((SSPCON2) and 07Fh), 0 bit ? hardware clears when START complete
    goto I2C_WaitStart ; loop until hardware clears ((SSPCON2) and 07Fh), 0
    BANKSEL PIR1 ; select bank containing PIR1
    bcf PIR1,3 ; clear ((PIR1) and 07Fh), 3 flag after START condition
...
I2C_Stop:
    BANKSEL PIR1 ; select bank containing PIR1
    bcf PIR1,3 ; clear ((PIR1) and 07Fh), 3 before generating STOP
    BANKSEL SSPCON2 ; select bank containing SSPCON2
    bsf SSPCON2,2 ; set ((SSPCON2) and 07Fh), 2 bit ? initiate STOP condition
I2C_WaitStop:
    btfsc SSPCON2,2 ; test ((SSPCON2) and 07Fh), 2 bit ? hardware clears when STOP complete
    goto I2C_WaitStop ; loop until hardware clears ((SSPCON2) and 07Fh), 2
...
I2C_WaitTX:
    BANKSEL PIR1 ; select bank containing PIR1
    btfss PIR1,3 ; test ((PIR1) and 07Fh), 3 ? set when transmission complete
    goto I2C_WaitTX ; loop until ((PIR1) and 07Fh), 3 set
    BANKSEL SSPCON2 ; select bank containing SSPCON2
    btfsc SSPCON2,6 ; test ((SSPCON2) and 07Fh), 6 ? 1 = NACK received
    goto I2C_NackStop ; NACK ? jump to immediate STOP handling
    BANKSEL PIR1 ; select bank containing PIR1
    bcf PIR1,3 ; clear ((PIR1) and 07Fh), 3 after successful ACK
...
Update_ADCs:
    ...
    movlw 0b00000001 ; ((ADCON0) and 07Fh), 0=1, ((ADCON0) and 07Fh), 1=0, CHS=000 (AN0)
    movwf ADCON0 ; select AN0
    call Delay_InterPacket ; acquisition time >20 탎
    bsf ADCON0,1 ; set ((ADCON0) and 07Fh), 1/DONE ? start conversion
Wait0:
    btfsc ADCON0,1 ; test ((ADCON0) and 07Fh), 1/DONE ? 1 while conversion in progress
    ...
    ; --- AN1 (channel 1) ---
    BANKSEL ADCON0 ; select bank containing ADCON0
    movlw 0b00000101 ; ((ADCON0) and 07Fh), 0=1, ((ADCON0) and 07Fh), 1=0, CHS=001 (AN1)
...
Wait1:
    ...
    ; --- AN2 (channel 2) ---
    BANKSEL ADCON0 ; select bank containing ADCON0
    movlw 0b00001001 ; ((ADCON0) and 07Fh), 0=1, ((ADCON0) and 07Fh), 1=0, CHS=010 (AN2)
...
Start:
    ...
    bsf TRISA,0 ; set ((PORTA) and 07Fh), 0 as input (potentiometer)
    bsf TRISA,1 ; set ((PORTA) and 07Fh), 1 as input
    bsf TRISA,2 ; set ((PORTA) and 07Fh), 2 as input
    BANKSEL TRISC ; select bank containing TRISC
    movlw 0b00011000 ; ((PORTC) and 07Fh), 3=SCL, ((PORTC) and 07Fh), 4=SDA must be inputs for open-drain
    movwf TRISC ; configure I2C pins

    BANKSEL ADCON1 ; select bank containing ADCON1
    movlw 0b00000000 ; ((ADCON1) and 07Fh), 7=0 ? left justified result
    movwf ADCON1 ; set ADC format
    BANKSEL ADCON0 ; select bank containing ADCON0
    movlw 0b00000001 ; ((ADCON0) and 07Fh), 0=1, channel set later
    movwf ADCON0 ; enable ADC module

    BANKSEL SSPSTAT ; select bank containing SSPSTAT
    movlw 0x80 ; ((SSPSTAT) and 07Fh), 7=1 required for I2C master mode
    movwf SSPSTAT ; configure MSSP status
    BANKSEL SSPCON ; select bank containing SSPCON
    movlw 0x28 ; ((SSPCON) and 07Fh), 5=1, master mode enabled
    ...
    movlw 10 ; ADC update every N main loops ? change here only ; load defined ADC update rate
...
Main_Loop:
    ...
    movlw 10 ; ADC update every N main loops ? change here only ; reload defined rate
