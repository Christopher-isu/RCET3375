...
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "traffic_lights.S" 2
;=================================================================
; Traffic Light Control for PIC16F883
; 4MHz XT oscillator, TMR0 for 0.2s ticks, IOC on RB0/RB1
; Outputs: RA0-RA2 (EW Red/Yel/Grn), RA3-RA5 (NS Red/Yel/Grn), active HIGH
; Inputs: RB0 (EW car), RB1 (NS car), active LOW, 20us pulses
;=================================================================
...
# 23 "traffic_lights.S" 2

; GPR Variables (Bank 0)
    PSECT udata
    state: DS 1 ; State machine (0=EW_G, 1=EW_Y, 2=NS_G, 3=NS_Y, 4=AllRed)
    tick_count: DS 1 ; Counts TMR0 overflows (12 = ~0.2s)
    state_timer:DS 1 ; Counts ticks per state (e.g., 25 for 5s green)
    ew_car_flag:DS 1 ; Bit 0: 1 if EW car detected
    ns_car_flag:DS 1 ; Bit 0: 1 if NS car detected
    W_TEMP: DS 1 ; ISR context save
    STATUS_TEMP:DS 1 ; ISR context save
...
init:
    ; Initialize PORTA (outputs, active HIGH)
    BANKSEL TRISA
    CLRF TRISA ; ((PORTA) and 07Fh), 0 -((PORTA) and 07Fh), 5 outputs
    BANKSEL PORTA
    MOVLW 0x3F ; All lights ON (active HIGH, all red for safety)
    MOVWF PORTA
    ; Initialize PORTB (inputs)
    BANKSEL TRISB
    MOVLW 0x03 ; ((PORTB) and 07Fh), 0, ((PORTB) and 07Fh), 1 inputs
    MOVWF TRISB
    ; Disable analog inputs on PORTA and PORTB
    BANKSEL ANSEL
    CLRF ANSEL ; Disable analog for PORTA/C
    CLRF ANSELH ; Disable analog for PORTB (((PORTB) and 07Fh), 0, ((PORTB) and 07Fh), 1, ((PORTB) and 07Fh), 4, ((PORTB) and 07Fh), 5)
    ; Initialize TMR0 (1:64 prescaler, Fosc/4 = 1MHz)
    BANKSEL OPTION_REG
    MOVLW 0x85 ; 1:64 prescaler (((OPTION_REG) and 07Fh), 2:((OPTION_REG) and 07Fh), 0=101)
    MOVWF OPTION_REG
    ; Initialize IOC
    BANKSEL IOCB
    MOVLW 0x03 ; Enable IOC on ((PORTB) and 07Fh), 0, ((PORTB) and 07Fh), 1
    MOVWF IOCB
    ; Initialize Interrupts
    BANKSEL INTCON
    MOVLW 0xA0 ; Enable ((INTCON) and 07Fh), 7, ((INTCON) and 07Fh), 5 (((INTCON) and 07Fh), 3 later)
    MOVWF INTCON
    ; Initialize Variables
    BANKSEL state
    MOVLW 4 ; Start in AllRed
    MOVWF state
    CLRF tick_count
    MOVLW 5 ; 1s AllRed (5 ticks)
    MOVWF state_timer
    CLRF ew_car_flag
    CLRF ns_car_flag
...
tick_handler:
    ; Clear ((INTCON) and 07Fh), 2
    BCF INTCON, 2
    ; Increment tick counter
    INCF tick_count, F
    MOVLW 12 ; 12 overflows = ~0.2s
    SUBWF tick_count, W
    BTFSS STATUS, 2
    RETURN
    ; 0.2s tick reached
    CLRF tick_count ; Reset tick counter
    ; Preload TMR0 for next cycle (fine-tune last overflow)
    MOVLW 0xCB ; 256-53 for ~3,392us
    MOVWF TMR0
    ; Check PORTB for sustained low
    BANKSEL PORTB
    BTFSS PORTB, 0 ; ((PORTB) and 07Fh), 0 low?
    BSF ew_car_flag, 0
    BTFSC PORTB, 0 ; ((PORTB) and 07Fh), 0 high? Clear and re-enable IOC
    CALL clear_ew_flag
    BTFSS PORTB, 1 ; ((PORTB) and 07Fh), 1 low?
    BSF ns_car_flag, 0
    BTFSC PORTB, 1 ; ((PORTB) and 07Fh), 1 high? Clear and re-enable IOC
    CALL clear_ns_flag
    ; Disable IOC until flags processed
    BANKSEL INTCON
    BCF INTCON, 3
...
update_state:
    ...
    GOTO state_all_red
    GOTO state_ew_green
    GOTO state_ew_yellow
    GOTO state_ns_green
    GOTO state_ns_yellow
state_all_red:
    MOVLW 0x3F ; All ON (active HIGH: ((PORTA) and 07Fh), 0 -((PORTA) and 07Fh), 5=1)
    GOTO set_lights
state_ew_green:
    MOVLW 0x0C ; ((PORTA) and 07Fh), 2=1 (EW Green), ((PORTA) and 07Fh), 3=1 (NS Red)
    GOTO set_lights
state_ew_yellow:
    MOVLW 0x0A ; ((PORTA) and 07Fh), 1=1 (EW Yellow), ((PORTA) and 07Fh), 3=1 (NS Red)
    GOTO set_lights
state_ns_green:
    MOVLW 0x21 ; ((PORTA) and 07Fh), 5=1 (NS Green), ((PORTA) and 07Fh), 0=1 (EW Red)
    GOTO set_lights
state_ns_yellow:
    MOVLW 0x11 ; ((PORTA) and 07Fh), 4=1 (NS Yellow), ((PORTA) and 07Fh), 0=1 (EW Red)
    GOTO set_lights
set_lights:
    BANKSEL PORTA
    MOVWF PORTA
...
next_state:
    ; Check flags and current state to decide next
    BANKSEL state
    MOVF state, W
    XORLW 4 ; AllRed?
    BTFSC STATUS, 2
...
to_ew_green:
    MOVLW 0
    MOVWF state
    MOVLW 25 ; 5s = 25 ticks
    MOVWF state_timer
    RETURN
check_ew_green:
    MOVF ew_car_flag, W
    BTFSC STATUS, 2 ; No EW car?
    GOTO to_ew_yellow
    MOVF ns_car_flag, W
    BTFSC STATUS, 2 ; No NS car?
    RETURN ; Stay in EW Green
to_ew_yellow:
    MOVLW 1
    MOVWF state
    MOVLW 5 ; 1s = 5 ticks
...
ioc_loop:
    ...
    BSF ns_car_flag, 0
    BANKSEL INTCON
    BTFSC INTCON, 0 ; Re-check if new change occurred
    GOTO ioc_loop
    BCF INTCON, 0
    RETURN
