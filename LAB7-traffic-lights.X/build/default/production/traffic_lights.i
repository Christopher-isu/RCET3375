...
; Blends cycling/timing/tables from V1.9 with IOC/interrupt/detection from second program
;
...
; No Timer0; extensions restart ticks. Fits vector options, no changes needed.
;=================================================================
...
ISR:
    ...
    btfsc PIR1, 0 ; ((PIR1) and 07Fh), 0 bit 0 set? (tick overflow)
    ...
    retfie ; Return from int, re-enable ((INTCON) and 07Fh), 7
...
HANDLE_IOC:
    ...
    ; Test EW car (((PORTB) and 07Fh), 0 low)
    btfsc PORTB, 0 ; ((PORTB) and 07Fh), 0 high? Skip if low (detected)
...
CHECK_NS_DETECT:
    btfsc PORTB, 1 ; ((PORTB) and 07Fh), 1 high? Skip if low (detected)
...
HANDLE_TIMER1:
    banksel PIR1 ; Switch to PIR1 bank
    bcf PIR1, 0 ; Clear ((PIR1) and 07Fh), 0 bit 0 to reset flag
...
LIGHT_EW_GREEN:
    movlw 0b100001 ; EW Green (((PORTC) and 07Fh), 0=1), NS Red (((PORTC) and 07Fh), 5=1)
    goto SET_LIGHT ; Jump to set outputs
LIGHT_EW_YELLOW:
    movlw 0b100010 ; EW Yellow (((PORTC) and 07Fh), 1=1), NS Red (((PORTC) and 07Fh), 5=1)
    goto SET_LIGHT ; Jump to set outputs
LIGHT_NS_GREEN:
    movlw 0b001100 ; NS Green (((PORTC) and 07Fh), 3=1), EW Red (((PORTC) and 07Fh), 2=1)
    goto SET_LIGHT ; Jump to set outputs
LIGHT_NS_YELLOW:
    movlw 0b010100 ; NS Yellow (((PORTC) and 07Fh), 4=1), EW Red (((PORTC) and 07Fh), 2=1)
SET_LIGHT:
...
; Disables unused to avoid issues. IOC on ((PORTB) and 07Fh), 0/((PORTB) and 07Fh), 1 for changes.
...
START:
    ...
    movlw 0xC0 ; ((PORTC) and 07Fh), 0 -5 outputs, ((PORTC) and 07Fh), 6 -7 inputs
    ...
    clrf ANSELH ; Disable analog on PORTB pins (((PORTB) and 07Fh), 0/((PORTB) and 07Fh), 1 digital)
    ...
    ; Setup IOC on ((PORTB) and 07Fh), 0/1
    banksel IOCB ; Switch to IOCB bank
    movlw 0b00000011 ; Enable IOC on bits 0 and 1 (((PORTB) and 07Fh), 0/((PORTB) and 07Fh), 1)
    ...
    bsf PIE1, 0 ; Enable ((PIE1) and 07Fh), 0 for timer interrupts
    banksel INTCON ; Switch to INTCON bank
    bsf INTCON, 6 ; Enable ((INTCON) and 07Fh), 6 for peripheral ints
    bsf INTCON, 7 ; Enable ((INTCON) and 07Fh), 7 for global interrupts
