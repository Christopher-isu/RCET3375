...
# 1 "C:/Users/chris/.mchp_packs/Microchip/PIC16Fxxx_DFP/1.7.162/xc8\\pic\\include/pic.inc" 1 3



# 1 "C:/Users/chris/.mchp_packs/Microchip/PIC16Fxxx_DFP/1.7.162/xc8\\pic\\include/pic_as_chip_select.inc" 1 3
# 379 "C:/Users/chris/.mchp_packs/Microchip/PIC16Fxxx_DFP/1.7.162/xc8\\pic\\include/pic_as_chip_select.inc" 3
# 1 "C:/Users/chris/.mchp_packs/Microchip/PIC16Fxxx_DFP/1.7.162/xc8\\pic\\include\\proc/pic16f883.inc" 1 3
# 47 "C:/Users/chris/.mchp_packs/Microchip/PIC16Fxxx_DFP/1.7.162/xc8\\pic\\include\\proc/pic16f883.inc" 3
INDF equ 0000h
...
EECON2 equ 018Dh
# 2658 "C:/Users/chris/.mchp_packs/Microchip/PIC16Fxxx_DFP/1.7.162/xc8\\pic\\include\\proc/pic16f883.inc" 3
...
# 380 "C:/Users/chris/.mchp_packs/Microchip/PIC16Fxxx_DFP/1.7.162/xc8\\pic\\include/pic_as_chip_select.inc" 2 3
# 5 "C:/Users/chris/.mchp_packs/Microchip/PIC16Fxxx_DFP/1.7.162/xc8\\pic\\include/pic.inc" 2 3
...
SERVO_POS_0: DS 1 ; Servo 0 position (0-63) ? ((PORTC) and 07Fh), 2/CCP1 hardware PWM
SERVO_POS_1: DS 1 ; Servo 1 position (0-63) ? ((PORTC) and 07Fh), 1/CCP2 hardware PWM
SERVO_POS_2: DS 1 ; Servo 2 position (0-63) ? ((PORTB) and 07Fh), 0/TMR2 software PWM
...
;-----------------------------------------------------------
; I2C INTERRUPT SERVICE ROUTINE ? ((PIE1) and 07Fh), 3 only
...
ISR:
    ...
    btfss PIR1,3 ; test ((PIR1) and 07Fh), 3 ? skip if I2C interrupt not active
    ...
    goto Store_Servo2 ; branch to Servo 2 storage (((PORTB) and 07Fh), 0)

    movlw 0xC0 ; load ID 11 comparator value
    subwf TEMP,W ; TEMP - 0xC0
    btfsc STATUS,2 ; Z=1 if TEMP=0xC0 ? Servo ID 11 (alias to Servo 2)
    goto Store_Servo3 ; store to Servo 2 (((PORTB) and 07Fh), 0)

    goto Release_Clock ; invalid ID ? ignore packet

Store_Servo0: ; Servo ID 00 ? ((PORTC) and 07Fh), 2/CCP1 hardware PWM
...
Store_Servo1: ; Servo ID 01 ? ((PORTC) and 07Fh), 1/CCP2 hardware PWM
    movf RECV_BYTE,W ; load clean 6-bit position (0-63)
    movwf SERVO_POS_1 ; store for CCP2 update
    goto Release_Clock ; release I2C clock stretch

Store_Servo2: ; Servo ID 10 ? ((PORTB) and 07Fh), 0/TMR2 software PWM ?
    movf RECV_BYTE,W ; load clean 6-bit position (0-63)
    movwf SERVO_POS_2 ; store for ((PORTB) and 07Fh), 0 TMR2 compare
    goto Release_Clock ; release I2C clock stretch

Store_Servo3: ; Servo ID 11 ? alias to Servo 2 (((PORTB) and 07Fh), 0)
    movf RECV_BYTE,W ; load clean 6-bit position (0-63)
...
Release_Clock: ; all valid packets reach here
    ...
    bsf SSPCON,4 ; set ((SSPCON) and 07Fh), 4=1 ? release clock stretch to master
...
Start:
    ...
    clrf ANSEL ; all digital I/O (((PORTA) and 07Fh), 0 -((PORTA) and 07Fh), 7)
    ...
    clrf SRCON ; disable SR latch (no ((CM2CON0) and 07Fh), 6/RxDT use)

    BANKSEL PORTA ; select bank containing PORT latches
    clrf PORTA ; initialise PORTA outputs low
    clrf PORTB ; initialise PORTB outputs low (((PORTB) and 07Fh), 0=LOW safe)
    ...
    movwf TRISA
    BANKSEL TRISB
    movlw 0b11111110 ; ((PORTB) and 07Fh), 0=output (Servo2 PWM), others input
    movwf TRISB
    BANKSEL TRISC
    movlw 0b11111000 ; ((PORTC) and 07Fh), 0=heartbeat, ((PORTC) and 07Fh), 1/2=Servo PWM outputs
    movwf TRISC ; ((PORTC) and 07Fh), 3/4=I2C open-drain inputs
    ...
    movlw 0b00000111 ; TOUTPS=111 (1:16 postscale), ((T2CON) and 07Fh), 2=1, T2CKPS=10
    movwf T2CON ; Timer2 running ? 16µs tick for CCP PWM

    BANKSEL CCP1CON ; select bank containing CCP1 control
    movlw 0x0C ; ((CCP1CON) and 07Fh), 3:0=1100 ? PWM mode
    movwf CCP1CON ; enable CCP1 PWM on ((PORTC) and 07Fh), 2
    movwf CCP2CON ; enable CCP2 PWM on ((PORTC) and 07Fh), 1 (same value)

    ; DC1B/DC2B fractional bits cleared ONCE (micro-optimisation)
    BANKSEL CCP1CON ; select CCP1CON again
    bcf CCP1CON,5 ; clear ((CCP1CON) and 07Fh), 5 ? 8-bit duty only
    bcf CCP1CON,4 ; clear ((CCP1CON) and 07Fh), 4
    BANKSEL CCP2CON ; select CCP2CON
    bcf CCP2CON,5 ; clear ((CCP2CON) and 07Fh), 5
    bcf CCP2CON,4 ; clear ((CCP2CON) and 07Fh), 4
    ...
    movwf SSPADD
    BANKSEL SSPSTAT ; select bank containing SSPSTAT
    movlw 0x80 ; ((SSPSTAT) and 07Fh), 7=1 ? slew rate control for 100kHz
    movwf SSPSTAT
    BANKSEL SSPCON ; select bank containing SSPCON
    movlw 0x36 ; ((SSPCON) and 07Fh), 6=0, ((SSPCON) and 07Fh), 7=0, ((SSPCON) and 07Fh), 4=1, ((SSPCON) and 07Fh), 5=1, I2C slave mode
    movwf SSPCON ; enable I2C slave with clock stretch

    BANKSEL PIE1 ; select bank containing peripheral IE
    bsf PIE1,3 ; enable ((PIE1) and 07Fh), 3 ? I2C interrupts only
    BANKSEL INTCON ; select bank containing global interrupt control
    bsf INTCON,7 ; ((INTCON) and 07Fh), 7=1 ? enable all interrupts
    bsf INTCON,6 ; ((INTCON) and 07Fh), 6=1 ? enable peripheral interrupts

;-----------------------------------------------------------
; MAIN LOOP ? decoupled PWM updates + rate limiting
; CCP1/2: every 4 loops (~1ms), ((PORTB) and 07Fh), 0: every loop (timing critical)
;-----------------------------------------------------------
Main_Loop:
    ...
    goto Do_RB0_Only ; not time for CCP update ? ((PORTB) and 07Fh), 0 only

    clrf UPDATE_COUNT ; reset rate limiter counter

    ; CCP1 update (Servo 0 - ((PORTC) and 07Fh), 2 hardware PWM)
    ...
    ; CCP2 update (Servo 1 - ((PORTC) and 07Fh), 1 hardware PWM)
...
Do_RB0_Only:
    ; STEP 1: ((PORTB) and 07Fh), 0 TMR2 POLLING ? PULSE WIDTH CONTROL (Servo 2 - ID 10)
    ; TMR2 compare ? HIGH when TMR2 < threshold (early pulse), LOW when late
    bcf INTCON,7 ; ((INTCON) and 07Fh), 7=0 ? disable interrupts (timing critical)
    ...
    bsf PORTB,0 ; EARLY cycle ? ((PORTB) and 07Fh), 0 HIGH (pulse active 1-2ms)
    btfsc STATUS,0 ; carry SET ? TMR2 >= threshold ? PULSE OFF
    bcf PORTB,0 ; LATE cycle ? ((PORTB) and 07Fh), 0 LOW (pulse inactive)
    bsf INTCON,7 ; ((INTCON) and 07Fh), 7=1 ? re-enable interrupts

    BANKSEL PORTC ; select bank containing PORTC
    bsf PORTC,0 ; ((PORTC) and 07Fh), 0=1 ? heartbeat pulse (scope monitor)
    bcf PORTC,0 ; ((PORTC) and 07Fh), 0=0 ? heartbeat complete (50% duty)
