...
resetVect:
    ...
    BANKSEL INTCON
    BTFSC INTCON,1 ; ((INTCON) and 07Fh), 1 set?
    GOTO HANDLE_INT
    BANKSEL PIR1
    BTFSC PIR1,0 ; ((PIR1) and 07Fh), 0 set?
    GOTO HANDLE_TMR1
    GOTO ISR_END ; Unexpected, exit

HANDLE_INT:
    BANKSEL INTCON
    BCF INTCON,1 ; Clear ((INTCON) and 07Fh), 1
    BCF INTCON,4 ; Disable ((INTCON) and 07Fh), 4 during timeout
    BANKSEL PORTC
    ...
    CLRF TMR1H
    BANKSEL PIE1
    BSF PIE1,0 ; Enable ((PIE1) and 07Fh), 0
    BANKSEL T1CON
...
HANDLE_TMR1:
    BANKSEL PIR1
    BCF PIR1,0 ; Clear ((PIR1) and 07Fh), 0
    BANKSEL COUNTER
    DECF COUNTER,F
    MOVF COUNTER,W
...
TIMEOUT:
    ...
    BANKSEL PIE1
    BCF PIE1,0 ; Disable ((PIE1) and 07Fh), 0
    BANKSEL PORTC
    MOVLW 0x31 ; Display '1'
    MOVWF PORTC
    BANKSEL INTCON
    BSF INTCON,4 ; Re-enable ((INTCON) and 07Fh), 4
    GOTO ISR_END
...
MAIN:
    CLRF PORTC ; Clear PORTC initially (((PORTC) and 07Fh), 7=0)
    BANKSEL TRISC
    CLRF TRISC ; Set PORTC as outputs
    BSF TRISB,0 ; Set ((PORTB) and 07Fh), 0 as input
    BANKSEL ANSELH
    BCF ANSELH,4 ; Clear ((ANSELH) and 07Fh), 4 for digital input on ((PORTB) and 07Fh), 0
    BANKSEL OPTION_REG
    BCF OPTION_REG,6 ; ((OPTION_REG) and 07Fh), 6=0: falling edge
    ...
    ; Check initial ((PORTB) and 07Fh), 0 state
    BTFSC PORTB,0 ; If high, enable INT and loop
    GOTO ENABLE_INT
    ; Else low: treat as pressed, display '7' and start timeout (((INTCON) and 07Fh), 4 remains disabled)
    MOVLW 0x37
    MOVWF PORTC
    MOVLW 4
    MOVWF COUNTER
    ...
    MOVWF T1CON
    BSF T1CON,0
    GOTO ENABLE_GIE ; Enable ((INTCON) and 07Fh), 7 (((PIE1) and 07Fh), 0 active)

ENABLE_INT:
    BANKSEL INTCON
    BCF INTCON,1 ; Clear ((INTCON) and 07Fh), 1
    BSF INTCON,4 ; Enable ((INTCON) and 07Fh), 4
ENABLE_GIE:
    BSF INTCON,7 ; Enable ((INTCON) and 07Fh), 7
