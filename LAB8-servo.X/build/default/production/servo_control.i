...
; Fosc/8), Timer1 (1:4), PORTC LEDs (((PORTC) and 07Fh), 0=LSB), ((PORTB) and 07Fh), 0 PWM out, runs
...
MAIN:
    ...
    BANKSEL ADCON1
    MOVLW 0b00000000 ; Set ADCON1=0 (((ADCON1) and 07Fh), 7=0: 8-bit, VCFG=00: VDD/VSS)
    MOVWF ADCON1 ; Configure ADC for left-justified, VDD ref
    BANKSEL ADCON0
    MOVLW 0b01000001 ; Set ADCON0=0b01000001 (Fosc/8, AN0, ((ADCON0) and 07Fh), 0=1)
    MOVWF ADCON0 ; Enable ADC, select AN0, 2µs TAD
    ; PORTC: 8-bit LED out (((PORTC) and 07Fh), 0=LSB, ((PORTC) and 07Fh), 7=MSB)
    BANKSEL TRISC
    ...
    BANKSEL T1CON
    MOVLW 0b00100000 ; Set T1CON=0b00100000 (1:4 prescale, ((T1CON) and 07Fh), 0=0)
    MOVWF T1CON ; Configure Timer1 for 4µs ticks, stopped
    ; ((PORTB) and 07Fh), 0: PWM output
    BANKSEL TRISB
    BCF TRISB, 0 ; Clear TRISB.0 to set ((PORTB) and 07Fh), 0 as output
    BANKSEL PORTB
    BCF PORTB, 0 ; Clear PORTB.0 to init ((PORTB) and 07Fh), 0 low
    ; Init vars
    BANKSEL OLD_ADC
...
DELAY_ACQ:
    ...
    BANKSEL ADCON0
    BSF ADCON0, 1 ; Set ((ADCON0) and 07Fh), 1/DONE=1 to start ADC conversion
WAIT_ADC:
    BTFSC ADCON0, 1 ; Check ((ADCON0) and 07Fh), 1/DONE, loop if 1 (wait ~22µs)
    ...
    ; Update LEDs
    BANKSEL PORTC
    MOVWF PORTC ; Set PORTC = ADC value (((PORTC) and 07Fh), 0=LSB)
...
; pulse on ((PORTB) and 07Fh), 0, off-time to 20ms using Timer1 (4µs ticks),
; polling ((PIR1) and 07Fh), 0 for software PWM flexibility, ((PORTB) and 07Fh), 0 usage
;=================================================================
GENERATE_PWM:
    BANKSEL PORTB
    BSF PORTB, 0 ; Set ((PORTB) and 07Fh), 0 high to start PWM pulse
    ; Preload = ~(PULSE) + 1
    ...
    BANKSEL PIR1
    BCF PIR1, 0 ; Clear ((PIR1) and 07Fh), 0 to reset Timer1 flag
    BANKSEL T1CON
    BSF T1CON, 0 ; Set ((T1CON) and 07Fh), 0=1 to start Timer1
WAIT_PULSE:
    BANKSEL PIR1
    BTFSS PIR1, 0 ; Check ((PIR1) and 07Fh), 0, loop if 0 (pulse active)
    GOTO WAIT_PULSE ; Wait for pulse duration
    BANKSEL T1CON
    BCF T1CON, 0 ; Clear ((T1CON) and 07Fh), 0 to stop Timer1
    BANKSEL PORTB
    BCF PORTB, 0 ; Clear ((PORTB) and 07Fh), 0 to end PWM pulse
...
WAIT_OFF:
    BANKSEL PIR1
    BTFSS PIR1, 0 ; Check ((PIR1) and 07Fh), 0, loop if 0 (off-time active)
